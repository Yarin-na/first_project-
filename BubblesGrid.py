...
def find_bubble_location_in_grid(bullet_bubble):
    ...
    for row in range(len(bubbles_grid)):
        for col in range(len(bubbles_grid[row])):
            distance = Bubble.distance_middles(bubbles_grid[row][col], bullet_bubble)
            if distance <= min:
                min = distance
                location = (row, col)
    return location
    ...
    for row in range(len(bubbles_grid)-1):
        for col in range(consts.BUBBLE_GRID_COLS):
            ...
            if bubble['color'] != consts.NO_BUBBLE:
                ...
                if Bubble.is_isolated(bubbles_grid, bubble_location):
                    isolated_locations.append(bubble_location)
    return isolated_locations

def grid_colors():
    list_of_colors = []
    for row in range(len(bubbles_grid)):
        for col in range(consts.BUBBLE_GRID_COLS):
            list_of_colors.append(bubbles_grid[row][col]['color'])
    return list_of_colors


def color_check(bubble_colors, list_of_colors):
    finished_colors = []
    for color in bubble_colors:
        if list_of_colors.count(color) == 0:
            if Stack.stack_colors().count(color) == 0:
                finished_colors.append(color)
    return finished_colors
